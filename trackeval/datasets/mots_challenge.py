...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['pedestrian', 'car','van','truck','bus' 'ignored_resions'] #, 'ignored resions'
        #self.valid_classes = ['ignored_resions', 'pedestrian','people', 'bicycle', 'car', 'van',
        #                               'truck', 'tricycle', 'awning_tricycle', 'bus',
        #                               'motor', 'others'] #, 'ignored resions'
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only pedestrian class is valid.')
        #self.class_name_to_class_id = {'ignored_resions': 0, 'pedestrian': 1, 'people': 2, 'bicycle': 3, 'car': 4, 'van': 5,
        #                               'truck': 6, 'tricycle': 7, 'awning_tricycle': 8, 'bus': 9,
        #                               'motor': 10, 'others': 11}
